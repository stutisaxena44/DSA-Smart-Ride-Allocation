...
**Binary Search — the decision flow** that powers every ID lookup and the mark-busy step:

```mermaid
flowchart TD
    A["low = 0<br/>high = total − 1"] --> B{low ≤ high?}
    B -->|No| F["Return −1<br/>(not found)"]
    B -->|Yes| C["mid = (low + high) / 2"]
    C --> D{"Compare<br/>pool[mid].id vs targetID"}
    D -->|Equal| E["Return mid<br/>(found ✅)"]
    D -->|target is larger| G["low = mid + 1<br/>search right half"]
    D -->|target is smaller| H["high = mid − 1<br/>search left half"]
    G --> B
    H --> B
```

**Insertion Sort — keeping the pool sorted**, traced on the three Andheri drivers as they load (IDs arrive out of order):

```text
Add 104          →  [104]
Add 101  key=101 →  104 > 101, shift  →  [101, 104]
Add 113  key=113 →  113 > 104, stop   →  [101, 104, 113]   ✓ already in place
```

Because only the newest driver is ever out of position, each insert costs `O(n)` in practice — far cheaper than re-sorting the whole pool.
...
The entire system runs on just two `struct` types held in fixed-size global arrays — no classes, no pointers, no dynamic allocation. A booking links a rider to the dispatched driver.

```mermaid
erDiagram
    DRIVER {
        int    id          "PK · unique, sort key"
        string name
    ...
    RIDE {
        int    rideID      "PK · auto-incremented"
        string riderName
        string driverName
        string pickup
        string destination
    }
    DRIVER ||--o{ RIDE : "dispatched into"
```

| Store | Type | Capacity | Guard | Invariant |
|-------|------|:--------:|-------|-----------|
| `pool[]` | `Driver` | `MAX = 50` | duplicate-ID check on insert | **always sorted by `id`** |
| `history[]` | `Ride` | `MAX_RIDES = 100` | bounds check before append | append-only, unique `rideID` |

> The "always sorted by `id`" invariant is the contract that lets **Binary Search** run — break it and lookups silently fail.
...
The high-level state of the program from launch to exit — the menu loop dispatches each choice to the algorithm that backs it, then returns:

```mermaid
flowchart TD
    Start([Launch]) --> Gate{Start or Exit?}
    Gate -->|0 · Exit| End([👋 Goodbye])
    Gate -->|1 · Start| Load["Load 15 sample drivers<br/>Insertion Sort keeps pool sorted"]
    Load --> Menu{Main Menu — choice?}
    Menu -->|1| V[View all drivers]
    Menu -->|2| A["Add driver · Insertion Sort"]
    Menu -->|3| R["Request ride · Bubble + Selection + Binary"]
    Menu -->|4| L["Lookup by ID · Binary Search"]
    Menu -->|5| S["Search by area · Linear Search"]
    Menu -->|6| M[Mark driver available]
    Menu -->|7| U[Refresh live locations]
    Menu -->|8| H[View ride history]
    V --> Menu
    A --> Menu
    R --> Menu
    L --> Menu
